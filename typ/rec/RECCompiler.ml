...
let _ =
  if not (Filename.check_suffix input_file ".rec") then 
    failwith "expected .rec extension"
let input = open_in input_file
let lexing_buffer = Lexing.from_channel input

let output_file = (Filename.chop_suffix input_file ".rec") ^ ".tpl"
let output = open_out output_file

let _ =
  try
    let source = VARParser.program VARLexer.token lexing_buffer in 
    let target = rec_to_tpl source in
